...
$categoryCode = $productResult[0]["category_id"];

$category_table = TB_CATEGORIES; 
$sql = "SELECT * FROM $category_table WHERE id = $cate";
$stmt = $db-> prepare($sql);
$stmt->execute();
$categoryResult = [];
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        <div class="flex-grow mr-0 md:mr-10 mb-5 md:mb-0">
          <div class="mb-5">
            <div class="w-6/12">
              ...
              <p class="px-2 py-2 border rounded-md"><?= $productResult[0]["code"] ?></p>
            </div>
          </div>
          ...
          <div class="flex justify-between mb-5">
            <div class="flex-grow mr-10">
              ...
              <p class="bg-white px-2 py-2 border  rounded-md"><?= $productResult[0]["category_id"] ?></p>
            </div>
            <div class="w-4/12">
              <p class="text-gray-500 text-left uppercase tracking-wider">price</p>
              <p class="bg-white px-2 py-2 border  rounded-md"><?= $productResult[0]["price"] ?></p>
            </div>
          </div>

          <div>
            <p class="text-gray-500 text-left uppercase tracking-wider">name</p>
            <p class="bg-white text-lg px-2 py-2 border  rounded-md"><?= $productResult[0]["name"] ?></p>
          </div>
        </div>
